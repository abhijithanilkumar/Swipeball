...
function loadBall()
{
	...
	ball.position.x = ball.scale * canvas.width/2;
	...
	ball.draw = function() {
		ctx.beginPath();
		ctx.arc(this.position.x,this.position.y,this.scale*50,0,2*Math.PI);
		ctx.closePath();
		ctx.fill();
	...
	entityDef = {
		...
		x : ball.position.x,
		y : ball.position.y,
		halfHeight : ball.scale*50,
		halfWidth : ball.scale*50,
		type : "dynamic",
		settings : {
			density : 0.1,
			friction : 0,
...
function loadMine()
{
		...
		var wallChoice = Math.random()*2;
		if(wallChoice > 1)
			mine.position.y = mine.scale;
		else
			mine.position.y = canvas.height - mine.scale;
